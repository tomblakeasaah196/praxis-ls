...
function fakeClient({ owns = true, ticket = null } = {}) {
  ...
  return {
    ...
    async query(sql, params) {
      ...
      if (/FROM dossier WHERE dossier_id/.test(text)) return { rows: owns ? [{ "?column?": 1 }] : [] };
      if (/FROM milestone_instance WHERE milestone_instance_id/.test(text)) {
        return { rows: [{ code: "CUSTOMS_RELEASED", dossier_id: params[0] === "bad-ms" ? "other-dossier" : "d1" }] };
      }
      if (/^\s*SELECT \* FROM q_ticket WHERE/i.test(text) || /FROM q_ticket\b/.test(text)) {
        return { rows: ticket ? [ticket] : [] };
      }
      if (/INSERT INTO "?q_ticket_reply"?/i.test(text)) {
        inserted.push({ table: "q_ticket_reply", sql: text, params });
        return { rows: [{ q_ticket_reply_id: "r1", is_internal: /true/.test(String(params)) }] };
      }
      if (/INSERT INTO "?q_ticket"?/i.test(text)) {
        inserted.push({ table: "q_ticket", sql: text, params });
        return { rows: [{ q_ticket_id: "t1", dossier_id: "d1", status: "OPEN" }] };
      }
...
describe("raising a ticket", () => {
  it("refuses a dossier that is not the client's", async () => {
    ...
    await expect(
      service.raise(c, { dossierId: "d1", subject: "Nosy", clientId: "someone-else" }),
  ...
  it("gives the same NOT_FOUND as a missing file, so existence is not disclosed", async () => {
    ...
    await expect(service.raise(c, { dossierId: "d1", subject: "x", clientId: "x" }))
      .rejects.toMatchObject({ status: 404 });
  });

  it("allows a staff-raised ticket, which carries no clientId", async () => {
    const c = fakeClient();
    const row = await service.raise(c, { dossierId: "d1", subject: "Phoned in" });
  ...
  it("refuses a milestone that belongs to a different file", async () => {
    const c = fakeClient();
    await expect(
      service.raise(c, { dossierId: "d1", milestoneInstanceId: "bad-ms", subject: "x" }),
...
describe("the thread", () => {
  it("never lets a client post an internal note, whatever the payload says", async () => {
    const c = fakeClient({ ticket: { q_ticket_id: "t1", dossier_id: "d1", status: "OPEN" } });
    await service.reply(c, { ticketId: "t1", body: "hello", fromClient: true, internal: true, clientId: "c1" });
  ...
  it("moves an OPEN ticket to IN_PROGRESS when we answer", async () => {
    ...
    expect(c.inserted.find((i) => i.table === "status").params[1]).toBe("IN_PROGRESS");
  });

  it("reopens a RESOLVED ticket when the client replies to it", async () => {
    const c = fakeClient({ ticket: { q_ticket_id: "t1", dossier_id: "d1", status: "RESOLVED" } });
    await service.reply(c, { ticketId: "t1", body: "still not released", fromClient: true, clientId: "c1" });
  ...
  it("leaves a RESOLVED ticket alone when WE add a closing note", async () => {
    const c = fakeClient({ ticket: { q_ticket_id: "t1", dossier_id: "d1", status: "RESOLVED" } });
    await service.reply(c, { ticketId: "t1", body: "for the record", internal: true });
    ...
    c.query = async (sql) => {
      if (/FROM q_ticket_reply/.test(String(sql))) {
        return { rows: [
          ...
          { q_ticket_reply_id: "r2", body: "red channel, do not tell them", is_internal: true },
        ] };
      }
      if (/FROM dossier WHERE dossier_id/.test(String(sql))) return { rows: [{ ok: 1 }] };
      return { rows: [ticket] };
    };
    const forClient = await service.detail(c, { ticketId: "t1", clientId: "c1", forClient: true });
