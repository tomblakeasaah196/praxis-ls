...
describe("§12.4 AI explanation — validation and cache hygiene", () => {
  ...
  /* ── The cache has to know which prompt wrote what ──────────────────────
   *
   * `prompt_version` was written on insert and never read. So an explanation
   * generated by an older prompt was served forever, and improving the prompt
   * reached only signatures nobody had asked about yet — which is exactly the
   * signatures nobody is looking at. The NestJS answers would have outlived
   * the NestJS prompt.
   */
  it("ASKS THE DATABASE FOR ITS OWN PROMPT VERSION, not just the signature", () => {
    const { svc, queries } = loadExplain({
      chat: async () => ({ provider: "deepseek", text: "fresh" }),
    });
    return svc.explain({ errorId: ERROR_ID, actorId: ACTOR }).then(() => {
      const read = queries.find((q) =>
        /SELECT[\s\S]*FROM platform\.error_explanation/.test(q.sql),
      );
      expect(read.sql).toMatch(/prompt_version = \$2/);
      expect(read.params[1]).toBe(svc.PROMPT_VERSION);
    });
  });

  it("regenerates rather than serving an explanation written by an older prompt", async () => {
    let calls = 0;
    jest.resetModules();
    jest.doMock(DB, () => ({
      query: async (sql, params) => {
        if (/INSERT INTO/s.test(sql)) return { rows: [] };
        if (/FROM platform\.error_explanation/s.test(sql)) {
          // The double answers like the database does: the row is v1, the
          // service asked for v2, so there is nothing to return.
          const stale = { explanation: "NestJS-era answer", generated_by: "gemini", model: "g", created_at: new Date().toISOString(), prompt_version: 1 };
          return { rows: params[1] === stale.prompt_version ? [stale] : [] };
        }
        return { rows: [ERROR_ROW] };
      },
    }));
    jest.doMock(LLM, () => ({
      chat: async () => {
        calls += 1;
        return { provider: "deepseek", text: "grounded answer" };
      },
    }));
    ...
    expect(calls).toBe(1);
    expect(out.explanation).toBe("grounded answer");
    expect(out.cached).toBe(false);
  });

  it("keys Redis by prompt version too, so a deploy cannot serve last week's prompt", async () => {
    const seen = [];
    jest.resetModules();
    jest.doMock(DB, () => ({
      query: async (sql) => (/FROM platform\.error_explanation/s.test(sql) ? { rows: [] } : { rows: [ERROR_ROW] }),
    }));
    jest.doMock(LLM, () => ({ chat: async () => ({ provider: "deepseek", text: "x" }) }));
    jest.doMock(REDIS, () => ({
      getClient: () => ({
        get: async (k) => { seen.push(k); return null; },
        set: async (k) => { seen.push(k); return "OK"; },
      }),
    }));

    const svc = require("../../src/services/platform/error-explain.service");
    await svc.explain({ errorId: ERROR_ID, actorId: ACTOR });

    expect(seen.length).toBeGreaterThan(0);
    for (const k of seen) expect(k).toContain(`v${svc.PROMPT_VERSION}:`);
  });
});
