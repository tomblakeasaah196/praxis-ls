...
module.exports = {
  ...
  writes: [
    ...
    { key: "transition_cash_request", service: (c, p) => service.transition(c, { id: p.cash_request_id, to: p.to, entityId: p.entity_id, date: p.date }), schema: validator.schemas.aiTransition, permission: { module: "MOD-49", action: "approve" }, confirm: true, describe: "Advance a cash request by id one step along DRAFT→SUBMITTED→VALIDATED→APPROVED (REJECTED from SUBMITTED, VALIDATED or APPROVED); disburse/justify are separate actions. Cannot skip states." },
