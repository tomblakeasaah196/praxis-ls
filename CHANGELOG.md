...
- **The primary AI chat provider is now a choice in the Platform Console, not two constants in the code.** Which vendor answered first was `PRIMARY = "deepseek"` / `FALLBACK = "gemini"` in `src/services/ai/llm.service.js`, so switching the deployment to Gemini meant a code change, four test edits and a deploy — while everything else about a vendor (key, endpoint, model, active) was already the console's to set. Integrations → AI providers now shows the chain a turn will walk (*Chat chain: DeepSeek → Google Gemini*), marks the current primary on its card and offers **Use as primary** on the other chat-capable one, behind a confirmation that names the outcome, because it changes which vendor every tenant is billed to from their next turn. The choice is one flagged row — `ai_vendor_credential.is_chat_primary` (`migrations/platform/0108_ai_chat_primary.sql`), a partial unique index keeping it to **one**, DeepSeek flagged as the incumbent so the console never shows "none" while every turn still goes to DeepSeek — set by `PUT /api/platform/ai-vendors/:vendor/chat-primary` (`settings.write`), in a transaction that clears the old flag before setting the new one (a single UPDATE can trip the index on the transient duplicate) and audits `ai_vendor.chat_primary_set` with the previous vendor. `llm.service.resolveChain` reads it **per call**, so a switch reaches every API instance on the next turn with no restart and no cache to invalidate; the fallback is not chosen separately but is the default chain minus the primary (`services/ai/chat-vendors.js`), so choosing Gemini makes DeepSeek the fallback rather than leaving nothing behind it (audit B2). A flag on a vendor that cannot answer a chat call (Groq, Embeddings) is refused at write time with a `422` and ignored at read time; an explicit `vendorName` from a caller still wins; a platform DB that cannot be asked yields the default chain and the turn is not lost. `checkVendorHealth` resolves the same chain and the boot log now says *primary "gemini" (chosen in the platform console)* or *(code default)*, so a primary that differs from the documented default reads as a decision rather than a bug. The same migration repairs the platform **Gemini row where it still carries the seeded values**: `0060` pointed it at Google's *native* `…/v1beta` endpoint — the exact shape `looksLikeNativeGemini` has flagged as "resolves but every call fails" since audit B2, meaning a DeepSeek outage degraded to the stub rather than to Gemini — and at `gemini-1.5-flash`, which Google has shut down. Both move to the OpenAI-compatible gateway and `gemini-2.5-flash` (GA, no shutdown date announced); an operator's own edits are left alone, `0060` is not touched (an edited applied file is a fleet-wide content-drift alarm that repairs nothing), and `GEMINI_MODEL`'s env default is aligned to the same model so the `.env` fallback and the console path stop running different models at a ~10× price gap. Vision (`vision.service`) reads the same row's model through the Google SDK, so CV reading and mail OCR on a stock deployment come off the dead model too. Per-token prices for the Gemini row are **not** re-seeded — the operator re-prices it in AI Control → Vendors. 21 rules in `tests/unit/ai-vendor-health.test.js` and `ai-vendor-fallback.test.js` (the chain swaps rather than shrinks, the first request really goes to Gemini's gateway, per-call read, streaming path, non-chat flag ignored, lookup failure) and 9 in `tests/unit/ai-vendor-chat-primary.test.js` (refusal before any write, statement order, rollback on a missing row, audit payload, inactive primary ignored).
