  ...
  res.innerHTML =
    ...
    + (wrongs.length
        ? '<div class="review">' + wrongs.map(function(r){
            return '<div><b>Ch ' + r.ch + ' &mdash; ' + stripTags(r.q) + '</b><br>'
                 + 'Your answer: ' + stripTags(r.yours) + '<br>'
                 + 'Correct: <b>' + stripTags(r.right) + '</b><br>'
                 + '<span style="color:#5A6675">' + stripTags(r.why) + '</span></div>';
          }).join('') + '</div>'
