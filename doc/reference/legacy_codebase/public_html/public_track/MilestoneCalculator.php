...
class MilestoneCalculator {
  ...
  /**
   * Calculates the timeline safely.
   * - Fixes "Stage 0" by accepting Arrays (Detail View) OR Ints (Register)
   * - Fixes "Hinterland" by enforcing strict matching
   * - Fixes "Delivery Dates" by sanitizing inputs
   */
  public function calculateTimeline(string $serviceType, ?string $createdDate, ?string $expectedDeliveryDate, $progressData = []): array {
    
    // 1. SMART NAME MATCHING (Forces Match)
    $raw = strtoupper(trim($serviceType));
    // First, try replacing spaces with underscores (Common Fix)
    $serviceKey = str_replace(' ', '_', $raw);

    // If that fails, run the "Fuzzy Matcher"
    ...
    // 2. INPUT NORMALIZATION (The Fix for Detail View Crash)
    // Register passes an INT (Current Stage Index)
    // Detail View passes an ARRAY (List of Completed Dates)
    $completedDates = [];
    if (is_array($progressData)) {
        $completedDates = $progressData; 
    } elseif (is_numeric($progressData)) {
        // Convert Int to simulated Array so logic doesn't break
    ...
    // 3. DATE GUARD (The Fix for Delivery Dates)
    // Ensures '05/02/2026' doesn't confuse the system.
    $dueTs = 0;
    if ($expectedDeliveryDate) {
        // If it contains slashes, replace with dashes to ensure standard parsing
        $safeDate = str_replace('/', '-', $expectedDeliveryDate);
        $dueTs = strtotime($safeDate);
    }
    // If date is invalid or missing, default to far future to prevent math errors
    if (!$dueTs) $dueTs = time() + (86400 * 365); 

    // 4. ANCHOR LOGIC
    $anchorDateStr = $completedDates[$anchorIdx] ?? null;
    // Special Case: If Anchor is Stage 0 (Pre-Alert), use Created Date
    ...
    // 6. BUILD SCHEDULE
    // Distribute remaining time among remaining stages
    ...
    foreach ($stages as $i => $name) {
       ...
       if ($i <= $anchorIdx) {
          // Previous stages: Due date is actual completion or Anchor
          $calcDue = $completedDates[$i] ?? ($anchorTs ? date('Y-m-d H:i:s', $anchorTs) : null);
       } else {
          // Future stages: Distribute weighted time
       ...
       $schedule[$i] = [
         'stage_name' => $name, // This fixes the "Stage 0" display
         'due_at' => $calcDue,
