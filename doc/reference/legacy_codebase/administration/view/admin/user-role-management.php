...
(function IAMPage(){
  ...
  // --- 3b. MISSING TABS LOGIC ---
  
  // Audit Log Loader
  async function loadAuditLogs() {
    const feed = $('audit-feed');
    if (!feed) return;
    feed.innerHTML = '<div class="text-center text-muted py-3">Loading logs...</div>';

    try {
      // Fetch from the API we created
      const res = await fetch(`../../api/security/audit_list.php`); 
      const data = await res.json().catch(() => ({}));
      
      if (!res.ok || !data.ok || !data.rows.length) {
        feed.innerHTML = '<div class="text-center text-muted py-4">No logs found.</div>';
        return;
      }

      feed.innerHTML = data.rows.map(log => {
        // Color code based on severity
        const borderClass = log.severity === 'CRITICAL' ? 'border-danger' : (log.severity === 'WARNING' ? 'border-warning' : 'border-primary');
        const icon = log.severity === 'CRITICAL' ? '<i class="fa-solid fa-circle-exclamation text-danger"></i>' : '<i class="fa-solid fa-check-circle text-success"></i>';
        
        return `
        <div class="p-3 border-start border-4 ${borderClass} bg-light mb-2 rounded shadow-sm">
            <div class="d-flex justify-content-between">
                <div>
                    <div class="small fw-bold text-uppercase text-muted">${log.action}</div>
                    <div class="fw-bold text-dark">${icon} ${escapeHtml(log.user)} <span class="fw-normal text-muted">performed action</span></div>
                    <div class="mt-1 small text-dark">${escapeHtml(log.details)}</div>
                </div>
                <div class="text-end">
                    <div class="small fw-bold font-monospace">${log.date}</div>
                    <div class="small text-muted">${log.ip}</div>
                </div>
            </div>
        </div>`;
      }).join('');
    } catch (e) {
      feed.innerHTML = `<div class="text-danger p-3">Error: ${escapeHtml(e.message)}</div>`;
    }
  }

  // Session Monitor Loader
  async function loadSessions() {
    ...
    try {
      ...
      if (!res.ok || !data.ok || !data.rows.length) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center py-4">No active sessions.</td></tr>';
        return;
      }
      ...
      tbody.innerHTML = data.rows.map(s => {
        const badge = s.status === 'ACTIVE' 
            ...
            : '<span class="badge bg-secondary bg-opacity-10 text-secondary">IDLE</span>';
        
        return `
        <tr>
            <td>
                <div class="fw-bold text-dark">${escapeHtml(s.user)}</div>
                <small class="text-muted">${escapeHtml(s.dept)}</small>
            </td>
            <td>${s.login}</td>
            <td class="font-monospace small">${s.ip} <span class="text-muted">(${s.device})</span></td>
            <td>${s.last_active}</td>
  ...
  try {
    ...
    // If connection closes / no response, this next line may throw — catch will handle it
    const text = await res.text();
    let data;
    try { data = JSON.parse(text); } catch(_) { data = { ok: false, message: text || 'Invalid JSON response' }; }

    if (!res.ok || !data.ok) {
      console.error('[PROVISION] server returned error:', res.status, data);
    ...
    console.error('[PROVISION] request failed:', e);
  ...
  document.addEventListener('DOMContentLoaded', () => {
    loadUserAccounts(); // Load the main table
    
    // NEW: Load other tabs when clicked
    document.getElementById('audit-tab').addEventListener('shown.bs.tab', loadAuditLogs);
    document.getElementById('sessions-tab').addEventListener('shown.bs.tab', loadSessions);

    // Initial load for them (optional, removes "empty" feel on first click)
    loadAuditLogs(); 
    loadSessions();
