...
<body>
   ...
   <nav class="sidebar collapsed">
    ...
    <div class="sidebar-menu accordion" id="adminMenu">
        ...
        <div class="accordion-item border-0">
            <button class="menu-btn" type="button" data-bs-toggle="collapse" data-bs-target="#admin4" aria-expanded="true">
...
function fmtDate(d) {
  ...
  /* ---------------------------
     Role & Permissions
   ---------------------------- */
  // This grabs the real role from PHP (e.g., 'SALES', 'FINANCE')
  const currentRole = '<?php echo $role; ?>'; 
  const isManager = ['ADMIN', 'MANAGEMENT'].includes(currentRole);
  ...
  <button onclick="openOffcanvas('view','${r.ref}')" class="btn btn-sm ${isManager ? 'btn-outline-dark' : 'btn-outline-primary'} fw-bold">
    <i class="fa-solid ${isManager ? 'fa-gear' : 'fa-eye'}"></i> 
    ${isManager ? 'Manage' : 'View File'}
  </button>
</td>
        </tr>
      `;
...
async function openOffcanvas(mode, ref) {
    ...
    // Logic for Manager/Read-only view
    if (!isManager && mode !== 'create') {
    ...
    // --- REMOVED DUPLICATE const DECLARATIONS HERE ---
    ...
    if (mode === 'create') {
        ...
        blocker.classList.add('d-none');
        btn.disabled = false;
        // Reset button text for new file
