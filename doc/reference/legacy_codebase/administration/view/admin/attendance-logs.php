...
<body>
  ...
  <div class="input-group input-group-sm">
    ...
    <input type="text" id="search-input" class="form-control" placeholder="Name, ID..." style="width: 180px;">
  </div>
</div>
...
<div class="d-flex align-items-end gap-1">
  <div>
    ...
    <input type="date" id="filter-start" class="form-control form-control-sm" style="width: 130px;">
  </div>
  <div class="align-self-center text-muted small pt-4">-</div>
  <div>
    <label class="form-label small fw-bold text-muted mb-1">To</label>
    <input type="date" id="filter-end" class="form-control form-control-sm" style="width: 130px;">
  </div>
</div>
...
<div>
  ...
  <select id="filter-status" class="form-select form-select-sm" style="width: 140px;">
    ...
    <option value="LATE" class="fw-bold text-danger">Late (!)</option>
...
<div>
  ...
  <select id="filter-dept" class="form-select form-select-sm" style="width: 150px;">
    ...
    <button id="btn-export-pdf" class="btn btn-outline-dark btn-sm fw-bold" type="button">
        <i class="fa-solid fa-file-pdf me-2"></i> Print Report
    </button>
</div>
...
(function AttendancePageDB(){
  const OFFICE_IP_PREFIX = "143.105.152.207";
...
let currentRows = [];
  ...
  async function loadAttendance(){
    ...
    const dept   = ($('filter-dept')?.value || '').trim();
    const status = ($('filter-status')?.value || '').trim(); // NEW
    const start  = ($('filter-start')?.value || '').trim();  // NEW
    const end    = ($('filter-end')?.value || '').trim();    // NEW

    // 2. Build URL
    const params = new URLSearchParams();
    if (q) params.set('q', q);
    if (dept) params.set('dept', dept);
    if (status) params.set('status', status); // Send to API
    if (start) params.set('start', start);    // Send to API
    if (end) params.set('end', end);          // Send to API
    ...
    // 4. Update KPIs (The "Analytics" Blend)
    // The API should calculate these totals based on the DATE RANGE you selected
  ...
  // --- UPDATED LISTENERS (Bottom of file) ---
  document.addEventListener('DOMContentLoaded', () => {
    // Set default date to TODAY so the view isn't empty initially
    const today = new Date().toISOString().split('T')[0];
    $('filter-start').value = today;
    $('filter-end').value = today;

    // Attach listeners to NEW inputs
    $('search-input')?.addEventListener('keyup', rerun);
    $('filter-dept')?.addEventListener('change', rerun);
    $('filter-status')?.addEventListener('change', rerun); // NEW
    $('filter-start')?.addEventListener('change', rerun);  // NEW
    $('filter-end')?.addEventListener('change', rerun);    // NEW
    
    // ... existing export/print listeners ...
...
function renderTable(rows){
    ...
    tbody.innerHTML = rows.map(r => {
      ...
      const ipIn = r.ip_in || '';
      // Check if actually present (Time In exists)
      if (timeIn !== '--') {
          // 1. Check Location
          if (isOfficeIp(ipIn)) {
              // On Site: Check if Late
              if (timeIn > LATE_CUTOFF) {
                  uiStatus = 'LATE';
              ...
              // Off Site
              uiStatus = 'REMOTE';
              statusClass = 'status-info'; // Blue badge for remote
              icon = '<i class="fa-solid fa-laptop-house"></i>';
          }
          
          // 2. Overwrite if Auto-Closed
          if ((r.attendance_status || '').includes('AUTO')) {
               uiStatus = 'AUTO-CLOSED';
      ...
      // Duration: Show "Active" if clocked in but not out
      let duration = (r.duration && r.duration !== '--') ? r.duration : '--';
      if (timeIn !== '--' && timeOut === '--') {
          duration = '<span class="text-primary fw-bold">Active...</span>';
      }

      const ipOut = r.ip_out || '--';
      const devIn  = r.device_in  || '';
      const devOut = r.device_out || '';
      const uaIn   = (r.user_agent_in || '').slice(0, 60);
      const uaOut  = (r.user_agent_out || '').slice(0, 60);
