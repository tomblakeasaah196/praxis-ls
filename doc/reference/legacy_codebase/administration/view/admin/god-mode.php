...
<div class="modal fade" id="eradicationSuccessModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content border-0 shadow-lg text-center p-4">
            <div class="mb-3">
                <i class="fa-solid fa-circle-check text-success fa-4x shadow-sm rounded-circle bg-white"></i>
            </div>
            <h5 class="fw-black text-dark mb-1">ERADICATION COMPLETE</h5>
            <p class="text-muted small fw-bold mb-3">Record permanently vaulted and destroyed.</p>
            
            <div class="bg-light rounded p-3 mb-4 border text-start">
                <div class="small text-muted text-uppercase fw-bold mb-1">Target Reference:</div>
                <div class="font-mono text-danger fw-bold fs-6 mb-2" id="successRef"></div>
                <div class="small text-muted text-uppercase fw-bold mb-1">Timestamp (WAT):</div>
                <div class="font-mono text-dark fw-bold" id="successTime"></div>
            </div>
            
            <button type="button" class="btn btn-success w-100 fw-bold" data-bs-dismiss="modal">Acknowledge</button>
        </div>
    </div>
</div>
        ...
        async function executeEradication() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    document.getElementById('successRef').innerText = state.targetRef;
                    document.getElementById('successTime').innerText = new Date().toLocaleString('en-GB', { hour12: false });
                    
                    const successModal = new bootstrap.Modal(document.getElementById('eradicationSuccessModal'));
                    successModal.show();
