...
Acct: 10005-0006-107018411001-93 | IBAN: CM21 1000 5000 6107</textarea>
  ...
  const res = await apiJson('/smart-logistics/administration/api/marginpricing/get-approved-costings.php', {
    method: 'GET'
  });
  ...
  renderLines();
    ...
    function handleEnter(e, idx, field) {
      if (e.key === 'Enter') {
        e.preventDefault();
        let nextId = '';
        if (field === 'sell') nextId = `note-${idx}`;
        else if (field === 'note') {
          if (idx + 1 < currentLines.length) nextId = `sell-${idx + 1}`;
          else { document.getElementById(`note-${idx}`).blur(); return; }
        }
          ...
          <tr>
            ...
            <td class="p-1 align-middle">
              <input type="number" class="form-control form-control-sm input-sell" id="sell-${i}"
                value="${sell}" oninput="updateLine(${i}, 'sell', this.value)"
            ...
            <td class="p-1 align-middle">
              <input type="text" class="form-control form-control-sm smart-input" id="note-${i}" placeholder="Notes for client..."
                value="${escapeHtml(l.remarks || '')}" oninput="updateLine(${i}, 'remarks', this.value)"
