...
require_role(['OPERATIONS']); // keep same guard as index.php
...
<head>
  ...
  <style>
    ...
    /* Fix for offcanvas scrolling issue */
    .offcanvas-body {
        overflow-y: auto;
        overflow-x: hidden;
    }
    /* Ensure tabs don't inherit weird display properties */
    form.tab-content {
        height: 100%;
        display: flex;
...
<body>
  ...
  <div class="offcanvas offcanvas-end offcanvas-custom" tabindex="-1" id="supplierDrawer">
    ...
    <div class="offcanvas-body p-4">
      <form id="supplier-form" class="tab-content" onsubmit="event.preventDefault(); saveSupplier();">
        ...
        <div class="tab-pane fade" id="evaluation" role="tabpanel">
          ...
          <div class="mb-4">
            ...
            <div class="bg-light p-3 rounded border">
              ...
              <div id="doc-input-digital"
                   class="border border-dashed bg-white p-3 text-center rounded"
                   role="button" tabindex="0"
                   style="cursor:pointer;"
                   onclick="document.getElementById('inp-doc-file').click()">
                <input
                  type="file"
                  id="inp-doc-file"
                  class="d-none"
                ...
                <small class="text-muted">
                  <i class="fa-solid fa-cloud-arrow-up"></i> Click to select PDF/JPG/PNG
                </small>
                <div id="doc-upload-status" class="small text-muted mt-2"></div>
              </div>

              <div id="doc-input-physical" class="hidden mt-2">
                <input type="text" id="inp-physical-ref" class="form-control form-control-sm"
                       placeholder="Archive Reference Number (Required)">
              </div>
              ...
              <div class="mt-3">
                <div class="small text-muted fw-bold mb-2">Queued documents (saved with supplier)</div>
  ...
  // Document queue (single-submit)
  let docQueue = []; // items: {document_type, storage_mode, file, physical_ref}
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    // FIX: Scroll to top of drawer content when switching tabs to prevent "lost content" issue
    const tabEl = document.getElementById('supplierTabs');
  ...
  async function editSupplier(id){
    try{
      ...
      // reset docs queue for this edit session
      docQueue = [];
      renderDocQueue();
      const status = document.getElementById('doc-upload-status');
      status && (status.innerText = '');
  ...
  // ---------- Documents (queued, uploaded only on Save Supplier) ----------
  ...
  function addDocumentRecord() {
    ...
    if (mode === 'PHYSICAL') {
      ...
      if (!ref) { alert('Physical archive selected: Archive Reference Number is required.'); return; }
  ...
  // ---------- Save Supplier (single submit; supplier + docs) ----------
  ...
  const payload = {
    ...
    // IMPORTANT: documents metadata must align indexes with doc_file_0, doc_file_1...
  ...
  // Attach files in the same index order as documents[]
  ...
  try {
    ...
    // clear docs after successful save
    docQueue = [];
    renderDocQueue();
