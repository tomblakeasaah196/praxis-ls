...
if (isset($_GET['ajax'])) {
  ...
  LEFT JOIN client_master cm
    ON TRIM(cm.client_id) = TRIM(ofm.client_id)
...
<body>
  ...
  <div class="top-navbar">
    <div>
      ...
      <small class="text-muted" style="font-size: 0.7rem;">FINANCE DEPARTMENT</small>
    </div>
  ...
  <div class="input-group">
    ...
    <input
      ...
      id="fileSearch"
      list="opsFileOptions"
      placeholder="Type Operations File Ref (e.g., SL--...)"
      oninput="debouncedLoadOpsFiles()"
      onchange="onOpsFilePicked()"
    >
...
async function loadOpsFiles(q){
  // Don’t hammer server for tiny input
  if (!q || q.length < 2){
    OPS_FILES = [];
  ...
  OPS_FILES.forEach(r => {
    ...
    opt.value = r.operations_file_reference; // visible value
    dl.appendChild(opt);
  });

  document.getElementById('opsfile-hint').textContent =
    OPS_FILES.length ? `Found ${OPS_FILES.length} matching operations files.` : 'No operations files found.';
}
...
async function onOpsFilePicked(){
  ...
  try{
    ...
    document.getElementById('linked-client-label').textContent =
      `${clientName || '—'} (${clientId || '—'})`;
    document.getElementById('linked-client-panel').classList.toggle('d-none', !(clientId || clientName));
  } catch(e){
    console.error('onOpsFilePicked failed:', e); // ADD THIS
    clearLinkedClient();
  }
}
