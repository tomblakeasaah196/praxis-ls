...
<body class="page employee-master">
  ...
  <div class="offcanvas offcanvas-end offcanvas-custom smart-offcanvas--medium smart-offcanvas--wide" tabindex="-1" id="empDrawer">
    ...
    <div class="offcanvas-body p-4 bg-light bg-opacity-25 d-flex flex-column justify-content-start">
      <form id="emp-form" class="tab-content h-auto w-100">
...
(function EmployeeMasterPage(){
  ...
  function switchRole(role){
    ...
    // label
    const roleLabel = (role === 'MANAGEMENT') ? 'SYSTEM ADMIN' : role;
  ...
  tbody.innerHTML = filtered.map(e => {
    ...
    const badgeClass = e.status === 'PENDING'
  ...
  try {
    ...
    // show status selector for edit
    $('status-toggle-container').classList.remove('hidden');

    const res = await fetch(`../../api/employees/get.php?employee_id=${encodeURIComponent(employeeId)}`, {
      headers: { 'Accept': 'application/json' },
      credentials: 'same-origin'
    });
    ...
    if (!res.ok || !data || !data.ok) {
      const msg = (data && (data.detail || data.message)) ? (data.detail || data.message) : (txt || `HTTP ${res.status}`);
      throw new Error(msg);
    }
    ...
    $('inp-dob').value = emp.dob || '';
    $('inp-marital').value = emp.marital || 'SINGLE';
    $('inp-email').value = emp.email || '';

    // Optional (only if you added these inputs)
    ...
    // Status
    $('inp-status').value = emp.status || 'PENDING';

    // Finance
    $('inp-salary').value = emp.salary ? String(emp.salary) : '';
    $('inp-pay-method').value = emp.payMethod || 'BANK_TRANSFER';
    $('inp-bank').value = emp.bank || '';

    // Authority (from employee_master.system_authority SET)
    const caps = Array.isArray(emp.authority) ? emp.authority.map(x => String(x).toUpperCase()) : [];
    ...
    // If you want Create Login Account default behaviour:
    // For existing employee, do not auto-create unless explicitly checked
    $('inp-linked-user').checked = false;
    ...
    console.error('editEmployee failed:', e);
    alert(e.message || 'Failed to load employee details');
  }
}
...
async function saveEmployee(){
  try {
    const payload = {
      ...
      status: (document.getElementById('inp-status')?.value || 'PENDING'),
    ...
    if (!payload.name) return alert('Full Legal Name is required');
    if (!payload.email) return alert('System Email is required');
    if (!payload.dept) return alert('Department is required');
    if (!payload.title) return alert('Job Title is required');
    if (!payload.joinDate) return alert('Join Date is required');

    console.info('Saving employee — payload:', payload);
    ...
    console.info('save.php status:', res.status, res.statusText);
    console.info('Response headers:', Array.from(res.headers.entries()));

    // Read raw text then try to parse JSON so we can show useful diagnostics
    const txt = await res.text();
    let json = null;
    try { json = txt ? JSON.parse(txt) : null; } catch (err) {
      console.warn('Response is not JSON:', txt.slice(0, 200));
    }

    if (!res.ok) {
      const msg = json && json.message ? json.message : txt || `HTTP ${res.status}`;
      console.error('Server returned error:', msg);
      alert('Save failed — server error: ' + msg);
      return;
    }

    if (!json || !json.ok) {
      const msg = json && json.message ? json.message : (txt || 'Unknown server response');
      console.error('Save failed — server response:', msg, json, txt);
      alert('Save failed: ' + msg);
      return;
    }

    console.info('Save successful:', json);
    await loadEmployees();
    empDrawer.hide();
    alert('Employee saved successfully.');

  } catch (err) {
    console.error('saveEmployee uncaught error:', err);
    alert('Save failed (client error): ' + (err.message || String(err)));
  }
}
