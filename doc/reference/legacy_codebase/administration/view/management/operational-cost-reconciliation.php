        ...
        <div class="accordion-item border-0">
            ...
            <div id="mgmt5" class="accordion-collapse collapse show" data-bs-parent="#mgmtMenu">
                <div class="sub-menu">
                    ...
                    <a href="operational-cost-reconciliation.php" class="sub-link active">Operational Cost Reconciliation</a>
                </div>
            </div>
        </div>
...
function renderRegisterFromDb(items){
  ...
  const isMgmt  = (CURRENT_ROLE === 'MANAGEMENT' || isAdmin);
  ...
  items.forEach(row => {
    ...
    } else if ((isFin || isMgmt) && status === 'SUBMITTED') {
      actionBtn =
...
function renderButtons() {
  ...
  if (isAdmin) {
    ...
    if (status !== 'DRAFT' && printBtn) printBtn.disabled = false;
    ...
    // Submitted → Show Finance / Management actions (Admin sees both)
    if (status === 'SUBMITTED') {
      container.innerHTML = `
  ...
  // --- MANAGEMENT ROLE (ONLY VALIDATE & LOCK) ---
  if (CURRENT_ROLE === 'MANAGEMENT') {
    if (status === 'SUBMITTED') {
      footerMsg.innerText = "Management Review: Approve & Lock.";
      container.innerHTML = `
        <button class="btn btn-success text-white" onclick="validateOCR(this)">Validate & Lock</button>
      `;
    } else {
