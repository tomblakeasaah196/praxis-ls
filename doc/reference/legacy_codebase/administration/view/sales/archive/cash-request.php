...
";
    ...
    foreach ($rawLines as $rl) {
      ...
      $exportLines[] = [
        ...
        'qty'             => (float)($rl['qty'] ?? 0),
        'unit_price'      => (float)($rl['total_ht'] ?? 0),
        'vat_rate'        => $vatRate,
  ...
  if ($ajax === 'pr_get') {
    ...
    $sqlL = "
      SELECT line_id, line_code, line_desc, qty, total_ht, vat_rate, line_total, is_imported, justification_required
    ...
    $logs = [];
    $logs[] = "Created by " . (string)($hdr['created_by'] ?? '-') . " on " . (string)($hdr['created_at'] ?? '-');
    if (!empty($hdr['updated_at']))   $logs[] = "Last updated by " . (string)($hdr['updated_by'] ?? '-') . " on " . (string)($hdr['updated_at'] ?? '-');
    if (!empty($hdr['validated_at'])) $logs[] = "Validated by " . (string)($hdr['validated_by'] ?? '-') . " on " . (string)($hdr['validated_at'] ?? '-');
    if (!empty($hdr['rejected_at']))  $logs[] = "Rejected by " . (string)($hdr['rejected_by'] ?? '-') . " on " . (string)($hdr['rejected_at'] ?? '-');
  ...
  if ($ajax === 'pr_transition') {
    ...
    if ($to === 'VALIDATED') {
      ...
      if (!in_array($from, ['SUBMITTED'], true)) json_exit(['ok' => false, 'error' => 'Invalid transition'], 409);
    ...
    if ($to === 'APPROVED_LOCKED') {
      // Only MD/Admin can approve
      if (!in_array($role, ['MANAGEMENT','ADMIN'], true)) {
        json_exit(['ok' => false, 'error' => 'Only MD/Admin can Approve'], 403);
      }
      if ($from !== 'VALIDATED') {
        json_exit(['ok' => false, 'error' => 'Must be Validated first'], 409);
      }
    ...
    if ($to === 'REJECTED') {
      if (!$isFinanceActor && $role !== 'MANAGEMENT') {
        json_exit(['ok' => false, 'error' => 'Permission denied'], 403);
      }
      if (!in_array($from, ['SUBMITTED','VALIDATED','DRAFT'], true)) {
        json_exit(['ok' => false, 'error' => 'Invalid transition'], 409);
      }
                            ...
                            <div class="col-md-5 border-start border-end px-4">
                                ...
                                <div id="ops-context">
                                    <div class="mb-3">
                                        ...
                                        <select class="smart-input fw-bold" id="opsFileSelect" onchange="checkCostingLink()">
