  ...
  <nav class="sidebar">
    ...
    <div class="sidebar-menu accordion" id="salesMenu">
        ...
        <div class="accordion-item border-0">
            ...
            <div id="sales7" class="accordion-collapse collapse show" data-bs-parent="#salesMenu">
                <div class="sub-menu">
                    <a href="documents-vault.php" class="sub-link active">Documents Vault</a>
                </div>
            </div>
        </div>
...
<div class="top-navbar">
  <div>
      ...
      <small class="text-muted" style="font-size: 0.7rem;">SECURE STORAGE & COMPLIANCE</small>
  </div>
                ...
                <table class="table table-custom mb-0 align-middle">
                    <thead>
                        <tr>
                            ...
                            <th>File Ref / Client</th> <th>Line Description</th>
                            <th>Beneficiary</th>
                            <th>Age</th>
              ...
              <div class="modal-body p-4">
                  ...
                  <form id="wizForm">
                      ...
                      <div class="step-pane d-none" id="step2">
                          <div id="wizSelectedSummary" class="alert alert-light border d-none mb-3">
                              <div class="d-flex justify-content-between align-items-center">
                                <div>
                                  <div class="fw-bold small text-uppercase text-muted">Selected Target Folder</div>
                                  <div class="fw-bold" id="wizSelectedFolderName">--</div>
                                  <div class="text-muted small font-mono" id="wizSelectedFolderId">--</div>
                                </div>
                                <button type="button" class="btn btn-sm btn-outline-secondary fw-bold" onclick="wizAllowFolderChange()">
                                  Change
                                </button>
                              </div>
                            </div>
  ...
  <script>
    ...
    // Wizard forced target (used by Resolve / Action Center)
    let wizardForce = {
      enabled: false,
      context: null,     // 'OPS' | 'OVH'
      folderId: null,    // folder ref/id to preselect
      folderName: null   // optional display label
    };
    ...
    // Tracks current opened repo folder (for smart upload prefill)
let activeFolderId = null;
let activeFolderName = '';
let activeRepoContext = 'OPS'; // 'OPS' | 'OVH'
 ...
 // Default view
    ...
    function clearRepoSelectionState() {
      // logical state
      activeFolderId = null;
      activeFolderName = '';
      activeRepoContext = document.getElementById('vmOvh')?.checked ? 'OVH' : 'OPS';
    
      // UI state: ensure we are not inside a folder view
      if (!document.getElementById('fileArea').classList.contains('d-none')) {
        closeFolder(); // hides fileArea and shows grid
      }
    }
    ...
    async function openFolder(folderId, folderName, context) {
        ...
        activeRepoContext = context; // 'OPS' or 'OVH'
    ...
    pageItems.forEach(f => {
        ...
        // NEW: Badge for Reference Number
        const refBadge = `<span class="badge bg-secondary mb-1 font-mono" style="font-size:0.7em">${f.doc_ref}</span>`;

        tbody.innerHTML += `
            <tr>
                <td class="small font-mono text-muted">${f.date}</td>
                <td>
                    ${refBadge}<br> <div class="fw-bold text-dark">${f.filename}</div>
    ...
    if (!json.data || json.data.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="text-center py-5 text-success fw-bold"><i class="fa-solid fa-check-circle fs-1 mb-2"></i><br>Compliance 100%. No Missing Evidence.</td></tr>';
        return;
    }
    ...
    json.data.forEach(m => {
        // --- PATCH START: Visibility Filter ---
        // We define privileged roles in JS
        const myRole = "<?php echo $role; ?>"; 
        const myId = "<?php echo $employeeId; ?>";
        const privileged = ['ADMIN','FINANCE','MANAGEMENT'];

        // Assuming API returns 'requisitor_id' or we match 'requisitor' name
        // If the data doesn't have ID, we might have to rely on name matching or backend fix (preferred).
        
        // If not privileged AND the requisitor name doesn't roughly match current user
        // (This is a fallback if API doesn't filter. Ideally, use Fix #2 above)
        if (!privileged.includes(myRole)) {
             // If the API returns a 'created_by' or 'requisitor_id', use that. 
             // If not, we skip rendering this row to hide it from UI.
             if (m.requisitor_id && m.requisitor_id !== myId) return; 
        }
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td>
                    <div class="fw-bold text-dark">${m.file_ref}</div>
                    <div class="text-xs text-muted text-uppercase">${m.client}</div>
                </td>
                <td>
                    <div class="small">${m.line_desc}</div>
                    <div class="text-xs text-muted">Req: ${m.requisitor}</div>
                </td>
                <td>${m.beneficiary}</td>
                <td><span class="debt-badge ${debtClass}">${m.debt_age}</span></td>
                <td class="text-end pe-4">
                    <button class="btn btn-sm btn-outline-primary fw-bold shadow-sm" 
                        onclick="initResolveWizard('${m.file_ref}', 'OPS', '${m.line_id}', '${m.line_desc.replace(/'/g, "\\'")}')">
    ...
    // Clear any previous auto-link data
    document.getElementById('wizardModal').dataset.autoLinkId = '';
    
    const ctxSel = document.getElementById('wizContext');
    const folderSel = document.getElementById('wizFolder');
    const sum = document.getElementById('wizSelectedSummary');

    // -------------------------------------------------------------------------
    // SCENARIO A: RESOLVE MODE (Forced by "Resolve" button)
    // -------------------------------------------------------------------------
    if (wizardForce.enabled) {
        // Pre-fill
        ctxSel.value = wizardForce.context || 'OPS';
        await wizLoadFolders();
        folderSel.value = wizardForce.folderId;

        // Lock inputs
        ctxSel.disabled = true;
        folderSel.disabled = true;

        // Store Auto-Link Data
        document.getElementById('wizardModal').dataset.autoLinkId = wizardForce.autoLinkId || '';

        // Show Summary
        if (sum) {
            let html = `<div class="fw-bold small text-uppercase text-muted">Target Folder</div>
                        <div class="fw-bold mb-2">${wizardForce.folderName}</div>`;
            
            if (wizardForce.autoLineDesc) {
                html += `<div class="alert alert-info py-2 px-3 mb-0 small border-info">
                            <i class="fa-solid fa-wand-magic-sparkles me-2"></i>
                            <strong>Auto-Resolve:</strong> Uploading will fix:<br> 
                            <em>"${wizardForce.autoLineDesc}"</em>
                         </div>`;
            }
            document.querySelector('#wizSelectedSummary > div > div').innerHTML = html;
            sum.classList.remove('d-none');
        }

        // IMPORTANT: Skip to Step 3 (Upload) immediately
        gotoStep(3); 
        onWizFolderChange(); 

        // Reset force flag
        wizardForce.enabled = false;
    ...
    // -------------------------------------------------------------------------
    // SCENARIO B: CONTEXT MODE (User is already inside a folder)
    // -------------------------------------------------------------------------
    // Check if fileArea is visible AND we have an active folder ID
    if (!document.getElementById('fileArea').classList.contains('d-none') && activeFolderId) {
        // Pre-fill based on current view
        ctxSel.value = activeRepoContext || 'OPS';
        await wizLoadFolders();
        ...
        if (sum) {
            document.querySelector('#wizSelectedSummary > div > div').innerHTML = `
                <div class="fw-bold small text-uppercase text-muted">Target Folder</div>
                <div class="fw-bold">${activeFolderName}</div>
                <div class="text-muted small font-mono">${activeFolderId}</div>
            `;
            sum.classList.remove('d-none');
        }

        // IMPORTANT: Skip to Step 2 (Details & Checkboxes)
        // We don't skip to Step 3 because the user might want to manually link lines
        gotoStep(2);
        onWizFolderChange(); // Load lines for step 2
    ...
    // -------------------------------------------------------------------------
    // SCENARIO C: DEFAULT MODE (Start from scratch)
    // -------------------------------------------------------------------------
    ctxSel.disabled = false;
    folderSel.disabled = false;
    if (sum) sum.classList.add('d-none');
    
    gotoStep(1);
    setTimeout(() => { wizLoadFolders(); }, 50);
    
    new bootstrap.Modal(document.getElementById('wizardModal')).show();
}


async function wizAllowFolderChange() {
  const ctxSel = document.getElementById('wizContext');
  const folderSel = document.getElementById('wizFolder');
...
function initResolveWizard(folderRef, ctx, lineId = null, lineDesc = null) {
    clearRepoSelectionState();
    
    const context = (ctx === 'OVH' || ctx === 'OPS') ? ctx 
                  : ((folderRef === 'Overhead' || folderRef === 'OVH') ? 'OVH' : 'OPS');
    
    wizardForce.enabled = true;
    wizardForce.context = context;
    wizardForce.folderId = folderRef;
    wizardForce.folderName = `Resolve Evidence - ${folderRef}`;
    
    // --- NEW: Store Auto-Link Data ---
    wizardForce.autoLinkId = lineId;     
    wizardForce.autoLineDesc = lineDesc; 
    ...
    // --- 1. RESTORED AUTO-LINK LOGIC ---
    // Check if the modal has a hidden Auto-Link ID (set by openWizard)
    const autoLinkId = document.getElementById('wizardModal').dataset.autoLinkId;
    let linesToSend = '';

    if (autoLinkId) {
        // CASE A: Automatic Mode (Resolving)
        linesToSend = autoLinkId; 
    } else {
        // CASE B: Manual Mode (Checkboxes)
        linesToSend = Array.from(document.querySelectorAll('.line-checkbox:checked')).map(c => c.value).join(',');
    }
    // -----------------------------------
    ...
    // Send the correct lines (Auto or Manual)
    fd.append('linked_lines', linesToSend); 
    ...
    try {
        const submitBtn = document.querySelector('#wizardModal .btn-success');
        ...
        if(json.status === 'success') {
            ...
            if(json.refs && json.refs.length > 0) {
                json.refs.forEach(item => {
                    listEl.innerHTML += `
                        <li class="list-group-item bg-white border mb-3 rounded shadow-sm p-3">
                            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                                <div style="min-width: 150px;">
                                    <div class="small text-muted text-uppercase fw-bold" style="font-size: 0.65rem; letter-spacing:1px;">Reference ID</div>
                                    <div class="small text-muted text-truncate" style="max-width: 200px;" title="${item.file}">
                                        <i class="fa-solid fa-file me-1"></i> ${item.file}
                                    </div>
                                </div>
                                <div class="d-flex align-items-center bg-light rounded px-3 py-2 border">
                                    <div class="fs-3 font-mono fw-black text-primary me-3" style="letter-spacing: 1px;">
                                        ${item.ref}
                                    </div>
                                    <button class="btn btn-white border shadow-sm" onclick="copyToClipboard('${item.ref}', this)" title="Copy to Clipboard">
        ...
        const submitBtn = document.querySelector('#wizardModal .btn-success');
        if(submitBtn) { submitBtn.disabled = false; submitBtn.innerHTML = 'Confirm Upload'; }
    }
}
...
function copyToClipboard(text, btnElement) {
    ...
    try {
        ...
        if (!res.ok || j.status !== 'success') {
            alert("Verify/Reject failed: " + (j.message || "Unknown error"));
            return;
        }
        ...
        // Refresh KPIs + Action Center
        loadKPIs();
        loadActionItems();

        // IMPORTANT: re-fetch current folder content so status updates in table
        if (!document.getElementById('fileArea').classList.contains('d-none')) {
            const fid = document.getElementById('openedFolderId').innerText;
            const ctx = document.getElementById('vmOps').checked ? 'OPS' : 'OVH';

            const fd2 = new FormData();
            fd2.append('action', 'fetch_content');
            fd2.append('folder', fid);
            fd2.append('context', ctx);

            const res2 = await fetch(API_ENDPOINT, { method: 'POST', body: fd2 });
            const j2 = await res2.json();

            repoData = Array.isArray(j2.data) ? j2.data : [];
            renderRepoTable();
        }

    } catch (e) {
        console.error(e);
        alert("System error while validating document.");
    }
}
    ...
    const isFileView = !document.getElementById('fileArea').classList.contains('d-none');
    
    if (isFileView) {
        // --- SEARCH MODE 1: Search Files inside an open folder ---
        ...
        // --- SEARCH MODE 2: Search Client Folders in the Grid ---
        const folders = document.querySelectorAll('#mainGrid .folder-card');
        let hasVisible = false;
        
        folders.forEach(f => {
            const text = f.innerText.toLowerCase();
            if (text.includes(val)) {
                f.style.display = 'flex'; // Keep visible
                hasVisible = true;
            } else {
                f.style.display = 'none'; // Hide
            }
        });

        // Optional: Show "No Results" message if grid is empty
        let noMsg = document.getElementById('grid-no-results');
        if (!hasVisible && val !== '') {
            if(!noMsg) {
                document.getElementById('mainGrid').insertAdjacentHTML('beforeend', '<div id="grid-no-results" class="col-12 text-center text-muted py-4">No folders found matching "' + val + '"</div>');
            }
        } else if (noMsg) {
            noMsg.remove();
        }
        ...
        <div class="modal-content text-center p-4">
            <div class="mb-3">
                <div class="rounded-circle bg-success bg-opacity-10 d-inline-flex align-items-center justify-content-center" style="width: 80px; height: 80px;">
                    <i class="fa-solid fa-check text-success display-4"></i>
                </div>
            </div>
            <h4 class="fw-bold mb-2">Upload Successful!</h4>
            <p class="text-muted small">The following documents have been secured in the vault.</p>
            
            <div class="card bg-light border-0 mb-4">
                <div class="card-body p-2" style="max-height: 200px; overflow-y: auto;">
                    <ul class="list-group list-group-flush bg-transparent text-start" id="successList">
                        </ul>
                </div>
            </div>
