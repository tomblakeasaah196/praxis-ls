...
$showMargin = in_array($role, ['ADMIN', 'FINANCE', 'MANAGEMENT']);
    ...
    <div class="sidebar-menu accordion" id="salesMenu">
        ...
        <div class="accordion-item border-0">
            ...
            <div id="sales5" class="accordion-collapse collapse show" data-bs-parent="#salesMenu">
                <div class="sub-menu">
                    <a href="operations-registry.php" class="sub-link active">Operations File Registry</a>
...
if (data.summary) {
  ...
  const elPending = document.getElementById('kpi-fin-pending');
  const elMargin = document.getElementById('kpi-margin');

  if (elTotal)   elTotal.innerText = data.meta.total_rows ?? 0;
  if (elActive)  elActive.innerText = data.summary.total_active ?? 0;
  if (elPending) elPending.innerText = data.summary.total_pending ?? 0;

  // Only update margin if the KPI exists (ADMIN/FINANCE/MANAGEMENT)
  if (elMargin) {
    elMargin.innerHTML =
      `${Number(data.summary.total_margin ?? 0).toLocaleString()} ` +
      `<span class="fs-6 fw-normal text-white-50">XAF</span>`;
  }
}
  ...
  let ref = params.get('ref') || params.get('ops') || params.get('open_ref') || null;
  const openMode = params.get('open') || null;

  if (!ref && window.location.hash) {
    const h = window.location.hash.replace('#','');
    // support #ref=SL123 or just #SL123
    if (h.startsWith('ref=')) ref = h.split('=')[1];
    else if (h.match(/^SL/i)) ref = h;
  }

  // 3) open create drawer
  if (openMode === 'create') {
    if (typeof openOffcanvas === 'function') {
      setTimeout(() => openOffcanvas('create'), 120);
    } else {
      console.warn('openOffcanvas not available to open create');
    }
    // remove param to avoid re-open on refresh
    try { history.replaceState(null, '', window.location.pathname + window.location.hash); } catch (e) {}
    return;
  }

  // 4) open view drawer if ref present
  if (ref) {
    if (typeof openOffcanvas === 'function') {
      setTimeout(() => openOffcanvas('view', ref), 120);
    } else {
      console.warn('openOffcanvas not available to open view for ref:', ref);
    }
    try { history.replaceState(null, '', window.location.pathname + window.location.hash); } catch (e) {}
  }
});
