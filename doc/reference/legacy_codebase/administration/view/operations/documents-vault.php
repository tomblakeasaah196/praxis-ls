...
<body>
    ...
    <div class="sidebar-menu accordion" id="opsMenu">
        ...
        <div class="accordion-item border-0">
            ...
            <div id="ops5" class="accordion-collapse collapse" data-bs-parent="#opsMenu">
                <div class="sub-menu">
                    ...
                    <a href="operational-milestone-tracking" class="sub-link">Operational Cost Reconciliation</a>
                </div>
            </div>
        </div>
        ...
        <div class="accordion-item border-0">
            <button class="menu-btn" type="button" data-bs-toggle="collapse" data-bs-target="#ops7">
                ...
                <i class="fa-solid fa-chevron-down menu-chevron show"></i>
            </button>
            <div id="ops7" class="accordion-collapse collapse" data-bs-parent="#opsMenu">
                <div class="sub-menu">
                    <a href="documents-vault" class="sub-link active ">Documents Vault</a>
                </div>
            </div>
        </div>
  ...
  <div class="top-navbar">
    <div>
      ...
      <small class="text-muted fw-bold" style="font-size: 0.65rem; letter-spacing: 0.5px;">SECURE STORAGE & COMPLIANCE</small>
    </div>
    ...
    <div class="d-flex align-items-center gap-4">
      ...
      <div class="d-flex align-items-center gap-3 ps-3 border-start">
        ...
        <img src="<?php echo $avatarUrl; ?>" class="rounded-circle shadow-sm border border-2 border-white" width="38" height="38" alt="Profile">
      </div>
    </div>
  </div>
...
let activeRepoContext = 'OPS'; // 'OPS' | 'OVH'
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // --- PATCH: Handle URL Redirects ---
        const params = new URLSearchParams(window.location.search);
        const filter = params.get('filter');
        
        if (filter === 'missing') {
            // 1. Switch visual tab
            switchMainTab('MISSING');
        } else if (filter === 'rejected') {
            switchMainTab('ACTION');
        } else {
            loadFolders('OPS'); 
        }

        // Always background fetch these counts
        ...
        fetch(`${API_ENDPOINT}?action=fetch_action_items`).then(r=>r.json()).then(d => {
            const el = document.getElementById('badgeActionCount');
            if(el) el.innerText = d.data.length;
        });
    });
    ...
    function renderMissingTable(json) {
        ...
        // --- PATCH: Check for Focus ID from Dashboard ---
        const params = new URLSearchParams(window.location.search);
        ...
        json.data.forEach(m => {
            ...
            // If this is the ID we clicked on dashboard:
            if (focusId && m.line_id == focusId) {
                rowClass = 'table-active border-start border-5 border-primary'; // Highlight row
                
                // Open Wizard automatically
                if (!autoTriggered) {
                    setTimeout(() => {
                        initResolveWizard(m.file_ref, 'OPS', m.line_id, m.line_desc.replace(/'/g, "\\'"));
                    }, 500); 
                    autoTriggered = true;
                }
            }
        ...
        if (focusId) {
            window.history.replaceState({}, document.title, window.location.pathname + "?filter=missing");
        }
    }
    ...
    json.data.forEach(item => {
        ...
        // --- THE FIX IS HERE ---
        // Old Line: <small class="text-muted">By ${item.uploader}</small>
        // New Line: <div class="small mt-1">${item.desc_html}</div>
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td>
                    ...
                    <div class="small mt-1">${item.desc_html}</div> ${noteHtml}
                </td>
                <td>${item.folder}</td>
                <td>${statusBadge}</td> <td class="text-end pe-4">${actionBtn}</td>
            </tr>
        `;
    });
}
    ...
    document.getElementById('wizardModal').dataset.autoLinkId = ''; // Reset
    ...
    if (wizardForce.enabled) {
        ...
        // Pass ID to Modal for Submit function
        ...
        gotoStep(3); // Skip to Upload
        wizardForce.enabled = false; // Reset
        new bootstrap.Modal(document.getElementById('wizardModal')).show();
        return;
    }

    // --- MODE B: CONTEXT (Inside a Folder) ---
    if (!document.getElementById('fileArea').classList.contains('d-none') && activeFolderId) {
        ...
        gotoStep(2); // Skip to Details
        onWizFolderChange();
        new bootstrap.Modal(document.getElementById('wizardModal')).show();
        return;
    }

    // --- MODE C: DEFAULT (Direct Upload) ---
    ctxSel.disabled = false;
    folderSel.disabled = false;
    if (sum) sum.classList.add('d-none');
    
    gotoStep(1); // Start at beginning
...
async function wizAllowFolderChange() {
  ...
  await wizLoadFolders();
    ...
    // INTELLIGENCE: Check for Auto-Link ID first
    const autoLinkId = document.getElementById('wizardModal').dataset.autoLinkId;
    let linesToSend = '';

    if (autoLinkId) {
        linesToSend = autoLinkId; // Use Silent ID
    } else {
        // Use Checkboxes (Manual Mode)
    ...
    if (dType === 'OTHER') dType = `Other (${document.getElementById('wizTypeOther').value})`;
    ...
    fd.append('linked_lines', linesToSend); // Send the ID (or empty string for Direct Upload)
    ...
    try {
                ...
                json.refs.forEach(item => {
                    listEl.innerHTML += `
                        <li class="list-group-item bg-white border mb-3 rounded shadow-sm p-3">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <div class="small text-muted text-uppercase fw-bold" style="font-size:0.65rem;">Reference ID</div>
                                    <div class="small text-muted text-truncate" style="max-width:200px;">${item.file}</div>
                                </div>
                                <div class="d-flex align-items-center bg-light rounded px-3 py-2 border">
                                    <div class="fs-3 font-mono fw-black text-primary me-3">${item.ref}</div>
                                    <button class="btn btn-white border shadow-sm" onclick="copyToClipboard('${item.ref}', this)"><i class="fa-regular fa-copy"></i></button>
                                </div>
                            </div>
                        </li>`;
                });
        ...
        document.querySelector('#wizardModal .btn-success').disabled = false;
    }
}
