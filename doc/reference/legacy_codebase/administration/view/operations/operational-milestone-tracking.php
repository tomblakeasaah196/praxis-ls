 ...
 * ------------------------------
 * Full Integrity Version: 3.1
 * Includes: Smart Logic, Retroactive Dates, Post-Updates, Traffic Light EDD
 */

require_once __DIR__ . '/../../includes/init.php';
require_once __DIR__ . '/../../includes/role_guard.php';

// FINANCE is blocked by UI Overlay (ISO-9001 Segregation of Duties)
...
<body>
        ...
        <div class="accordion-item border-0">
            ...
            <div id="ops3" class="accordion-collapse collapse show" data-bs-parent="#opsMenu">
                <div class="sub-menu">
                    ...
                    <a href="operational-milestone-tracking" class="sub-link active">Operational Milestone Tracking</a>
                </div>
            </div>
        </div>
  ...
  <div class="top-navbar">
    <div>
      ...
      <small class="text-muted" style="font-size: 0.7rem;">FEED THE SMART TRACK</small>
    </div>
          ...
          <table class="table mb-0 table-custom">
            <thead>
              <tr>
                ...
                <th style="min-width:120px;">Est. Delivery</th>
            ...
            <form onsubmit="event.preventDefault();">
              ...
              <div class="mb-3">
                <label class="form-label small fw-bold text-dark">
                    <i class="fa-regular fa-calendar-check me-1"></i> Completion Date
                </label>
                <input id="inp-completed-at" type="datetime-local" class="form-control smart-input" />
                <div class="form-text small" style="font-size:0.7rem">Leave as-is for "Now", or change for past events.</div>
              </div>
              
              <div class="d-flex gap-2">
                  <button type="button" class="btn btn-outline-primary fw-bold flex-grow-1" onclick="saveMilestone(0)">
                    <i class="fa-solid fa-pen-to-square me-2"></i>Update Info
                  </button>

                  <button type="button" class="btn btn-success fw-bold flex-grow-1" onclick="saveMilestone(1)">
  ...
  <script>
    ...
    // TRAFFIC LIGHT DATE LOGIC (Point 5)
    function getTrafficLightHtml(expectedStr, status) {
        if (!expectedStr) {
            return `<span class="badge bg-light text-muted border">No ETA</span>`;
        }
        
        const due = new Date(expectedStr);
        const now = new Date();
        const dateText = due.toLocaleDateString('en-GB'); // DD/MM/YYYY
        
        // Logic: Is the file closed?
        const isDelivered = ['OPERATIONALLY_COMPLETED', 'FINANCIALLY_PENDING', 'CLOSED'].includes(status);

        if (isDelivered) {
             return `<span class="badge bg-success text-white border-success" title="Delivered"><i class="fa-solid fa-check"></i> ${dateText}</span>`;
        }
        ...
        if (diffDays < 0) {
            ...
            // On Track -> Light Green/Blue (Safe)
            badgeClass = 'bg-light text-dark border-secondary'; 
        }

        return `<span class="badge ${badgeClass} ${pulseClass}" style="min-width:85px;">${dateText}</span>`;
    }

    function statusPillFromComputed(computedStatus, isDelayed){
      if (computedStatus === 'CLOSED') return `<span class="tag-pill tag-closed"><i class="fa-solid fa-circle-check"></i> Closed</span>`;
      
      // Point 10 + Traffic Light Logic
      if (isDelayed || computedStatus === 'DELAYED') return `<span class="tag-pill bg-danger text-white ops-pulse-red"><i class="fa-solid fa-triangle-exclamation"></i> LATE</span>`;
      ...
      if (computedStatus === 'AWAITING Est. Del. Date') return `<span class="tag-pill bg-light border text-muted">Awaiting ETA</span>`;
    ...
    async function refreshFiles(){
      ...
      files = (rows || []).map(r => {
        ...
        return {
          ...
          is_risk: r.is_risk || false,
          is_delayed: r.is_delayed || false,
          ...
          expected_delivery_time: r.expected_delivery_time || null // Point 5 Requirement
        };
      });
    ...
    function renderFiles(){
      ...
      const filtered = files.filter(f => {
        ...
        if (status === 'RISK')   matchStatus = (f.computed_status === 'RISK' || f.is_delayed);
      ...
      tbody.innerHTML = filtered.map((f) => {
        ...
        // Generate Traffic Light HTML for EDD Column (Point 5)
        const eddHtml = getTrafficLightHtml(f.expected_delivery_time, f.operations_status);

        return `
          <tr class="align-middle">
            <td class="fw-bold" style="font-family: monospace;">${escapeHtml(f.ref)}</td>
            <td class="fw-bold text-dark">${escapeHtml(f.client)}</td>
            <td>${eddHtml}</td> <td><span class="tag-pill tag-service"><i class="fa-solid fa-ship"></i> ${escapeHtml(SERVICE_LABEL[f.type] || f.type)}</span></td>
            ...
            <td>${statusPillFromComputed(f.computed_status, f.is_delayed)}</td>
            <td class="text-end">
    ...
    function renderTimeline(){
      ...
      milestones.forEach((m) => {
        ...
        // "Active" logic: Matches non-linear edits + Anchor logic
        let status = 'pending';
        if (isCompleted) status = 'completed';
        else if (m.status === 'pending' && m.due_at) status = 'active'; // Calculated active by Brain
        else if (!isCompleted && idx === currentIdx) status = 'active'; // Fallback

        // Anchor Visualization: If it's a future estimated stage (no anchor yet), it might be distinct
        const isFuture = !isCompleted && !m.due_at;
        ...
        const isClickable = (canEdit && file.computed_status !== 'CLOSED');
        const cursorStyle = isClickable ? 'cursor:pointer;' : 'cursor:default;';

        const completedInfo = isCompleted
          ? `<div class="small text-success">Completed: ${escapeHtml(new Date(m.completed_at).toLocaleString('en-GB'))}</div>`
          : '';

        const dueInfo = m.due_at
          ? `<div class="small text-muted">Due: ${escapeHtml(new Date(m.due_at).toLocaleString('en-GB', {day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit', hour12:false}))}</div>`
          : (isFuture ? '<div class="small text-muted fst-italic">Pending...</div>' : '');
        ...
        html += `
          <div class="timeline-item d-flex gap-3" style="${cursorStyle}" ${isClickable ? `onclick='selectStage(${idx})'` : ''}>
            <div class="timeline-dot ${status} ${isFuture ? 'bg-light' : ''}">${icon}</div>
            <div class="pt-2 flex-grow-1">
              ...
              ${dueInfo}
              ${completedInfo}
              ${m.notes ? `<div class="mt-1 small text-muted bg-light p-1 border rounded"><i class="fa-solid fa-note-sticky text-warning"></i> ${escapeHtml(m.notes)}</div>` : ''}
    ...
    function selectStage(idx){
      ...
      // Flaw C Fix: Retroactive Date Input
      const dtLocal = document.getElementById('inp-completed-at');
      if (m?.completed_at) {
        // Format existing DB date for datetime-local input
    ...
    // SAVE MILESTONE (DB)
    // =========================
    async function saveMilestone(markCompleted){
      ...
      const completedAt = document.getElementById('inp-completed-at')?.value?.replace('T',' ') || '';
      ...
      try {
        await apiPost('save_milestone.php', {
          ...
          mark_completed: markCompleted // 0 = Update Info Only, 1 = Complete
        });
        ...
        // alert("Saved Successfully."); // Optional feedback
      } catch (err) {
        console.error(err);
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    const admin4 = document.getElementById('admin4'); // LOGISTICS OPERATIONS collapse panel
    if (!admin4) return;

    // Open the LOGISTICS OPERATIONS accordion section by default
    admin4.classList.add('show');

    // Ensure the button shows expanded state (chevron / aria)
    const btn = document.querySelector('[data-bs-target="#admin4"]');
    if (btn) {
...
document.addEventListener("DOMContentLoaded", function() {
    // 1. Check if 'ref' exists in the URL
    ...
    if (targetRef) {
        ...
        const checkInterval = setInterval(() => {
            // Check if 'files' array is populated (it's a global variable in your script)
            if (typeof files !== 'undefined' && files.length > 0) {
                
                // 3. Find the file in the loaded list
                const fileExists = files.find(f => f.ref === targetRef);
                
                if (fileExists) {
                    // 4. Trigger the existing function to open the drawer
                    openTracker(targetRef);
                    clearInterval(checkInterval); // Stop checking
                } else {
                    // Stop checking after 5 seconds if not found (prevents infinite loop)
