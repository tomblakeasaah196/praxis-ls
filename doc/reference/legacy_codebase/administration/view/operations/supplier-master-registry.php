...
<body>
  ...
  <nav class="sidebar">
    ...
    <div class="sidebar-menu accordion" id="opsMenu">
        ...
        <div class="accordion-item border-0">
            ...
            <div id="ops1" class="accordion-collapse  show" data-bs-parent="#opsMenu">
                <div class="sub-menu">
                    <a href="client-master-registry" class="sub-link">Client Master Registry</a>
                    <a href="supplier-master-registry" class="sub-link active">Supplier Master Registry</a>
  ...
  <div class="offcanvas offcanvas-end offcanvas-custom" tabindex="-1" id="supplierDrawer" data-bs-backdrop="static">
      ...
      <form id="supplier-form" class="tab-content" onsubmit="event.preventDefault();">
        ...
        <div class="tab-pane fade" id="evaluation" role="tabpanel">
          ...
          <div class="form-check mt-4 border-top pt-3 border-danger">
            ...
            <label class="form-check-label small fw-bold text-danger d-none" for="inp-deactivate ">Deactivate Vendor (Remove from AVL)</label>
          </div>
        </div>
      </form>
    </div>
    ...
    <div class="p-4 border-top bg-white d-flex justify-content-end gap-2">
      ...
      <button type="button" id="btn-save" onclick="saveSupplier()" class="btn btn-dark fw-bold px-4 d-none">Save Supplier</button>
    </div>
  </div>
  ...
  async function renderTable() {
    ...
    tbody.innerHTML = listToRender.map(s => {
      ...
      const canEdit = (CURRENT_USER_ROLE !== 'FINANCE');
...
const btnSave = document.getElementById('btn-save'); // Target the Save button

if (CURRENT_USER_ROLE === 'OPERATIONS') {
    btnCreate?.classList.remove('d-none');
    btnSave?.classList.remove('d-none'); // This reveals the Save button for you
} else {
