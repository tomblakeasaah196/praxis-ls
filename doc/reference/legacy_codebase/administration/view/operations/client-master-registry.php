...
require_role(['OPERATIONS']); // keep as-is (you can widen later if needed)
...
// --- Safe display values ---
$fullName  = $me['full_name'] ?: 'Operations';
$firstName = trim(explode(' ', $fullName)[0] ?? 'Operations');
 ...
 <nav class="sidebar">
    ...
    <div class="sidebar-menu accordion" id="opsMenu">
        ...
        <div class="accordion-item border-0">
            ...
            <div id="ops1" class="accordion-collapse collapse show" data-bs-parent="#opsMenu">
                <div class="sub-menu">
                    <a href="client-master-registry" class="sub-link active">Client Master Registry</a>
...
<div class="col-xl-3 col-md-6 finance-only">
  <div class="card-custom p-4 d-flex align-items-center border-start border-4 border-danger">
    ...
    <div>
      ...
      <div class="kpi-value text-danger fs-4">
    ...
    <div class="card-custom p-4 mb-4">
      <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
        <div class="d-flex gap-1">
          ...
          <button onclick="filterType('SHIPPER')" class="btn btn-outline-secondary btn-sm fw-bold rounded-pill px-2 filter-btn">Shipper</button>
          <button onclick="filterType('CONSIGNEE')" class="btn btn-outline-secondary btn-sm fw-bold rounded-pill px-2 filter-btn">Consignee</button>
          <button onclick="filterType('BOTH')" class="btn btn-outline-secondary btn-sm fw-bold rounded-pill px-2 filter-btn">Both</button>
          <button onclick="filterType('OVERDUE')" class="btn btn-outline-danger btn-sm fw-bold rounded-pill px-2 filter-btn">
        ...
        <div class="d-flex gap-2">
          <div class="input-group input-group-sm" style="width: 220px;">
            ...
            <input type="text" id="search-input" onkeyup="renderTable()" class="form-control" placeholder="Search...">
          </div>
          <button onclick="openDrawer('new')" id="btn-create" class="btn btn-dark btn-sm fw-bold shadow-sm d-flex align-items-center gap-1 text-nowrap">
  ...
  <div class="offcanvas offcanvas-end offcanvas-custom" tabindex="-1" id="clientDrawer" data-bs-backdrop="static">
        ...
        <div class="tab-pane fade show active" id="identity" role="tabpanel">
          <div class="d-flex justify-content-between align-items-center p-3 bg-light rounded border mb-4">
            <div>
              ...
              <input type="hidden" id="inp-client-id" value="SLAS-CL-AUTO">
<div class="font-monospace fw-bold" id="inp-system-id">SLAS-CL-AUTO</div>
...
// This will actively SHOW the button for Operations
if (CURRENT_USER_ROLE === 'OPERATIONS') {
   document.getElementById('btn-create')?.classList.remove('hidden');
   document.querySelectorAll('.finance-only').forEach(el => el.classList.remove('hidden'));
}
...
function renderKpis(k) {
  ...
  // Update Green
  if (recvEl) recvEl.textContent = Number(k?.receivables || 0).toLocaleString();
  
  // Update Red
  if (overEl) overEl.textContent = Number(k?.overdue || 0).toLocaleString();
}
...
async function loadClients() {
  ...
  // LOGIC: If user wants OVERDUE, ask API for 'ALL' so we get everyone, then we filter in JS
  const typeToSend = (activeFilter === 'OVERDUE') ? 'ALL' : activeFilter;
  
  if (typeToSend && typeToSend !== 'ALL') params.set('type', typeToSend);
  if (q) params.set('q', q);

  try {
    const res = await fetch(`${CLIENT_LIST_API}?${params.toString()}`);
...
function renderTable() {
  ...
  const filtered = clients.filter(c => {
    ...
    if (activeFilter === 'OVERDUE') {
        // Only show if overdue amount > 0
        matchType = (Number(c.overdue || 0) > 0);
    } else if (activeFilter !== 'ALL') {
        matchType = (String(c.type || '').toUpperCase() === activeFilter);
    }
  ...
  if (filtered.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7" class="p-4 text-center text-muted fst-italic">No clients found.</td></tr>`;
      return;
  }
  ...
  tbody.innerHTML = filtered.map(c => {
    ...
    // --- START PATCH: Stacked Finance Display ---
    ...
    const financeCell = `
        <div class="d-flex flex-column align-items-end lh-1">
            <div class="text-success fw-bold">${Number(receivables).toLocaleString()}</div>
            ${overdue > 0 ? `<div class="text-danger small fw-bold mt-1" style="font-size:0.75rem;">${Number(overdue).toLocaleString()} Overdue</div>` : ''}
        </div>
    `;
    ...
    return `
      <tr>
        ...
        <td class="text-end finance-only fw-bold">${financeCell}</td>
        <td>${statusBadge}</td>
...
async function saveClient() {
  const idDisplay = document.getElementById('inp-system-id');

  // Capture the ID FIRST (before changing UI)
  const rawClientId = (idDisplay?.innerText || '').trim();
  const isNew = rawClientId.includes('AUTO') || rawClientId.includes('NEW');

  // UI-only feedback AFTER capturing value
  if (isNew) {
    idDisplay.dataset.originalIdText = rawClientId;
    idDisplay.innerHTML = '<i class="fa-solid fa-dice fa-spin me-2"></i> Generating ID...';
  }

  const fd = new FormData();

  // Use the captured value, not the mutated DOM value
  fd.append('client_id', rawClientId);
    ...
    try {
        ...
        if (text.trim().startsWith('<')) {
            ...
            alert('Server error (HTML returned). Check console.');
            return;
        }
