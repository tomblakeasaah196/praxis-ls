...
const SERVICE_CATEGORIES = ["SEA_FREIGHT_IMPORT","SEA_FREIGHT_EXPORT","AIR_FREIGHT_IMPORT","AIR_FREIGHT_EXPORT","HINTERLAND_TRANSIT","INLAND_TRANSPORTATION","END_TO_END_AIR_FREIGHT","WAREHOUSING",
"END_TO_END_SEA_FREIGHT","BUSINESS_REPRESENTATION"];
...
function render() {
  ...
  <td class="small">
    ${
      r.attachment_stored_name
        ? `<a class="fw-bold text-primary text-decoration-none"
             href="../../uploads/quote_requests/${encodeURIComponent(r.attachment_stored_name)}"
             target="_blank" rel="noopener">
             <i class="fa-solid fa-paperclip me-1"></i>${r.attachment_original_name ? String(r.attachment_original_name) : "Open"}
           </a>`
  ...
  <td class="text-end pe-4">
    <button class="btn btn-sm btn-outline-dark fw-bold btnView" data-id="${r.quote_request_id}">Manage</button>
  </td>
`;
...
async function openDrawerFromDb(id) {
  ...
  if (chips) {
    ...
    (j.attachments || []).forEach(a => {
      const url = a.url || (a.stored_name ? `/upload/quotes_request/${encodeURIComponent(a.stored_name)}` : "");
      if (!url) return;
      ...
      link.target = "_blank";
      link.rel = "noopener";
      link.innerHTML = `<i class="fa-solid fa-paperclip me-1"></i>${a.original_name || "Open attachment"}`;
