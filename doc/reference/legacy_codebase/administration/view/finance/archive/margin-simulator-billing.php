...
// --- Display values ---
$fullName  = trim((string)($me['full_name'] ?? 'FINANCE'));
...
<nav class="sidebar">
    ...
    <div class="sidebar-menu accordion" id="financeMenu">
        ...
        <div class="accordion-item border-0">
            ...
            <div id="fin3" class="accordion-collapse collapse show" data-bs-parent="#financeMenu">
                <div class="sub-menu">
  ...
  <div class="top-navbar">
    <div>
      <h5 class="mb-0 fw-bold text-dark">Margin Simulation and Quoting</h5>
      <small class="text-muted" style="font-size: 0.7rem;">SIMULATE YOUR MARGIN BEFORE QUOTING</small>
    </div>
...
Acct: 10005-0006-107018411001-93</textarea>
  ...
  <script>
   ...
   async function loadApprovedCostingsOnce(force = false) {
       if (window.__approvedCostingsLoaded && !force) return;
...
async function openSim(simRef, forceReload = false) {
  ...
  if (simRef === 'new') {
    // PATCH: Lazy Creation - Don't call API yet.
    // Just initialize the UI with a blank state.
    const dummySim = {
      id: 0,              // 0 indicates "Not Saved Yet"
      simulation_ref: 'NEW-SIM',
      status: 'DRAFT'
    };
    setupSimUI(dummySim, [], true);
    bsOffcanvas.show();
    // Do NOT call renderDashboard yet (nothing to show)
    return;
  }
  ...
  try {
    ...
    // FIX: data is only defined here; keep hydration logic inside this try, after const data
...
// If backend returned no lines (common after SUBMIT), rebuild from costing
const noLines = !Array.isArray(lines) || lines.length === 0;

// If lines exist but have no numeric amounts, also rebuild
const amountsMissing = Array.isArray(lines) && lines.length > 0 && lines.every(l => {
  const c = Number(l.cost_total_xaf ?? l.cost_xaf ?? l.total_cost_xaf ?? l.total_ht ?? 0);
  const s = Number(l.selling_total_xaf ?? l.sell ?? 0);
  return (!Number.isFinite(c) || c === 0) && (!Number.isFinite(s) || s === 0);
});

if (sim?.costing_id && (noLines || amountsMissing)) {
  // IMPORTANT: hydrateOnly:false so currentLines is rebuilt with amounts
  await loadCostingData(sim.costing_id, { hydrateOnly: false });

  // now pass empty lines into setupSimUI; currentLines is already ready
  lines = [];
} else if (sim?.costing_id) {
  // keep your existing lightweight hydration for SSDC/print fields
  await loadCostingData(sim.costing_id, { hydrateOnly: true });
}
...
setupSimUI(sim, lines, false);
  ...
  return false;
    ...
    function setupSimUI(sim, lines, isNew) {
      ...
      // 2. Display ISO Verification Hash (Challenge 34)
      ...
      // 3. Process Lines (Challenge 6 & 7 - Empty Pricing)
      currentLines = (lines || []).map(l => ({
        line_id: l.line_id || l.id || null,
        code: l.code || l.item_code || '',
        desc: l.description || l.item_description || l.desc || '',
        
        // Cost: Always respect backend value
        cost: Number(l.cost_total_xaf ?? l.cost_xaf ?? l.total_cost_xaf ?? l.cost ?? 0),
        
        // Sell: If 0 (or null), set to null so Input is empty. If value exists, use it.
        sell: (Number(l.selling_total_xaf ?? l.sell ?? 0) === 0) ? null : Number(l.selling_total_xaf ?? l.sell),
        
        remarks: l.quote_remarks || l.remarks || '',
       printOnQuote: !((l.client_facing ?? l.print_on_quote) === false || toBool(l.client_facing) === false),
applyVat: toBool(l.apply_vat ?? l.applyVat ?? l.vat_applicable),
isAdHoc: toBool(l.is_adhoc ?? l.isAdHoc),

      }));

      // 4. Fallback: If no lines but costing linked, load costing lines (Client Side Hydration)
      if ((!currentLines || currentLines.length === 0) && currentSim.costing_id) {
        ...
        if (c && Array.isArray(c.lines) && c.lines.length) {
          currentLines = c.lines.map(x => ({
            ...
            sell: null, // Force empty for manual input (Challenge 6)
            remarks: '',
            printOnQuote: true,
            applyVat: false,
            isAdHoc: false,
      ...
      // 6. Costing Selector & Unlock Button (Challenge 5)
      const sel = document.getElementById('costing-selector');
      
      // Ensure the "Unlock" button exists next to the selector
      let unlockBtn = document.getElementById('btn-unlock-costing');
      if (!unlockBtn && sel && sel.parentElement) {
          unlockBtn = document.createElement('button');
          unlockBtn.id = 'btn-unlock-costing';
          unlockBtn.className = 'btn btn-outline-danger btn-sm ms-2 fw-bold';
          unlockBtn.innerHTML = '<i class="fa-solid fa-link-slash"></i>';
          unlockBtn.title = 'Unlink/Reset Costing (Allows re-selection)';
          unlockBtn.style.display = 'none'; // Hidden by default
          
          unlockBtn.onclick = () => {
              if(isLocked) return;
              if(confirm("Unlink Costing? This will clear all current lines.")) {
                  sel.disabled = false;
                  sel.value = "";
                  unlockBtn.style.display = 'none';
                  resetSSDC();
                  currentLines = [];
                  if(currentSim) currentSim.costing_id = null;
                  renderLines();
              }
          };
          
          sel.parentElement.appendChild(unlockBtn);
      }

      // Configure Selector State
      if (isNew || !currentSim.costing_id) {
        if(sel) { sel.value = ""; sel.disabled = false; }
        if(unlockBtn) unlockBtn.style.display = 'none';
        resetSSDC();
      } else {
  // Ensure option exists even if get-approved excludes it
  const c = APPROVED_COSTINGS[currentSim.costing_id] || {};
  const label =
    `${currentSim.costing_ref || c.costing_ref || 'COSTING'} | ` +
    `${currentSim.client_name_cached || c.client_name || c.client_name_cached || '-'} | ` +
    `${currentSim.operations_file_reference || c.operations_file_reference || '-'} | ` +
    `${Number(c.totals?.total_ttc || c.total_ttc || 0).toLocaleString()} XAF`;
  ...
  renderSSDC(currentSim.costing_id);
      ...
      // 7. Global Locking Rules (View Only modes)
      isLocked = (status === 'APPROVED' || status === 'QUOTED');
      if (status === 'SUBMITTED') isLocked = true;
  ...
  currentLines = (data.lines || []).map(l => {
    // 1. COST: Always grab Total HT. Fallback to 0 if missing. Never grab TTC.
    const costHt = Number(l.total_ht || 0);
    
    // 2. VAT: Strict check. If backend says 1/true, we turn it ON.
    ...
    return {
      ...
      // Sell Default: Cost HT + 30% Margin
      sell: Math.ceil(costHt * 1),
  ...
  renderLines();
    ...
    function renderLines() {
      ...
      tbody.innerHTML = (currentLines || []).map((l, i) => {
        ...
        // Logic: If l.sell is null (from new line or reset), treat as 0 for calc, but empty for input
        const sellVal = (l.sell === null || l.sell === '') ? 0 : Number(l.sell);
        const margin = sellVal - cost;
        const pct = sellVal > 0 ? (margin / sellVal * 100) : 0;
        ...
        // KPI Logic (Challenge 2)
        let kpiClass = 'obs-fair';
        let kpiText = 'FAIR';
        if (pct < 10) {
          kpiClass = 'obs-poor'; kpiText = 'POOR';
        } else if (pct >= 35) {
          kpiClass = 'obs-excel'; kpiText = 'EXCEL';
        } else if (pct >= 20) {
          kpiClass = 'obs-good'; kpiText = 'GOOD';
        }
        ...
        // Input Value: Empty string if null, otherwise the number
  ...
  ${
    ...
    ? `
      <input
        type="text"
        class="form-control form-control-sm smart-input fw-bold"
        placeholder="Search fee in Financial Dictionary..."
        list="fin-dict"
        value="${escapeHtml(l.desc || '')}"
        oninput="updateAdHocDesc(${i}, this.value)"
        ${disabled}
      >
      <div class="text-muted font-mono mt-1" style="font-size:0.7rem">
        ${escapeHtml(l.code || '')}
      </div>
    `
    : `
    ...
    // If typing remarks, we don't need to do math
    if(field === 'remarks') return; 

    // 2. Do the Math manually for this one row
    ...
    // 3. Update the Margin Cell Text & Color (By ID)
    const margEl = document.getElementById(`cell-margin-${idx}`);
    if(margEl) {
        margEl.innerText = money(margin);
        
        // Remove old color classes and add the correct one
        margEl.classList.remove('text-danger', 'text-success');
        if(margin < 0) { 
            margEl.classList.add('text-danger'); 
        } else { 
            margEl.classList.add('text-success'); 
        }
    }

    // 4. Update the KPI Badge (By ID)
    ...
    // 5. Update the Totals at the bottom of the screen
    // We loop through the data to get the total, but we DO NOT redraw the HTML
    let tCost = 0, tRev = 0;
    ...
    // Update the bottom chart/bars
    updateSnapshot(tCost, tRev);
    
    // IMPORTANT: notice we do NOT call renderLines() here!
}
  ...
  const isFinance = String(currentRole || '').toUpperCase() === 'FINANCE';


const canSalesActions = isSales || isAdmin || isFinance;
const canMgmtActions  = isMgmt  || isAdmin || isFinance;
  ...
  // PATCH: Handle First Save (Lazy Create)
  if (currentSim.id === 0) {
    try {
      // Create the shell record first so we have an ID
      ...
      // We don't need to reload the whole UI, just update the ID so the next step works
    } catch (e) {
      alert("Failed to initialize simulation: " + e.message);
      document.getElementById('autosave-status').innerHTML = '<span class="text-danger">Creation Failed</span>';
      return;
    }
  }

  // Proceed with standard Save Logic (Update)
  try {
    const payload = {
      id: currentSim.id, // Now guaranteed to be > 0
      ...
      lines: (currentLines || []).map(l => ({
        ...
        cost_total_xaf: Number(l.cost || 0),
        selling_total_xaf: Number(l.sell || 0),
