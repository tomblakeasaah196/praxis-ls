...
function can(role, action) {
  ...
  const policy = {
    ...
    VALIDATE: ['ADMIN', 'MANAGEMENT', 'LEAD', 'FINANCE'], 
...
function renderDrawerActions() {
    ...
    let btnClose = `<button type="button" class="btn btn-light fw-bold text-muted" data-bs-dismiss="offcanvas">Close</button>`;
    let btnSave = '', btnSubmit = '', btnValidate = '', btnApprove = '', btnReject = '', btnUnlockRequest = '', btnManagementUnlock = '';

    // 2. CREATOR STAGE (Draft or Rejected)
    if (status === 'DRAFT' || status === 'REJECTED') {
        btnSave = `<button type="button" class="btn btn-dark fw-bold" onclick="saveDraft()"><i class="fa-regular fa-floppy-disk me-2"></i>Save</button>`;
        if (can(role, 'SUBMIT')) {
    ...
    // 3. VALIDATION STAGE (Finance/Lead Review)
    // We only show Reject here if the user is actually allowed to Validate
    ...
    // 4. APPROVAL STAGE (Management/Admin Review)
    // We only show Reject here if the user is actually allowed to Approve
    ...
    // 6. Render to UI (Ensure btnReject is placed next to the action buttons)
    box.innerHTML = [
        btnClose,
        btnSave,
        btnSubmit,
        btnValidate,
        btnApprove,
        btnReject, // Placed here to appear on the right side of actions
        btnUnlockRequest,
        btnManagementUnlock
    ].filter(Boolean).join('');
}
