...
<div class="toast-container" id="toastContainer"></div>
  ...
  <div class="offcanvas offcanvas-end" tabindex="-1" id="proformaEditor" data-bs-backdrop="static" style="width: 95vw; max-width: 1600px;">
    ...
    <span class="input-group-text fw-bold bg-warning text-dark border-warning">Advance %</span>
    <input type="number" 
           class="form-control text-center fw-bold border-warning bg-warning bg-opacity-10" 
           id="edAdvancePct" 
           value="" 
           placeholder="?" 
           min="1" 
...
if (wfStatus === 'REJECTED') {
    ...
    document.getElementById('edAdvancePct').value = p.invoice_id ? (p.advance_percentage || '') : '';
...
` + result.items.map(item => {
    ...
    container.innerHTML = html;
            ...
            const pctInput = document.getElementById('edAdvancePct');
    const pctValue = parseFloat(pctInput.value);

    if (isNaN(pctValue) || pctValue <= 0 || pctValue > 100) {
        utils.showToast('Error', 'Please enter a valid Advance Percentage (1-100%)', 'error');
        pctInput.focus(); // Jump cursor to the box
        pctInput.classList.add('is-invalid'); // Add red border temporarily
