...
// Guarantee a strict 1 or 0 regardless of what JS sends (true/false, "1", "0", etc)
$justificationRequired = isset($data['justification_required'])
    ? (int) !!$data['justification_required']
    : 0;
...
try {
  ...
  INSERT INTO financial_dictionary (
    ...
    receipt_required, receipt_source, justification_required,
...
// param types (16 params): 11 strings and 4 integers (is_negotiable, is_billable, justification_required, created_by)
$types = "ssssssssiississi"; // 16 chars
$stmt->bind_param(
  $types,
  $code, $nameEn, $nameFr,
  ...
  $receiptRequired, $receiptSource, $justificationRequired,
  $vatTreatment, $status,
  $userId
);
...
// param types (15 params): 11 strings + 3 ints (is_negotiable,is_billable, justification_required) + final int id
$types = "sssssssiississi"; // 15 chars
$stmt->bind_param(
  $types,
  $nameEn,
  $nameFr,
