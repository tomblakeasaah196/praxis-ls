...
try {
  ...
  if ($cur['status'] !== 'SUBMITTED') throw new RuntimeException("Only SUBMITTED can be approved");

  $stmt = $conn->prepare("
    UPDATE marginpricing_simulations
    SET status='APPROVED', approved_by_user_id=?, approved_at=NOW(), locked_at=NOW()
    WHERE id=?
  ");
  ...
  $stmt = $conn->prepare("
    ...
    SELECT id, simulation_ref, 'APPROVED', ?, ?, 'Approved'
  ...
  $conn->commit();
  echo json_encode(['ok'=>true,'message'=>'Approved']);
} catch (Throwable $e) {
  $conn->rollback();
