...
function initOpsDashboard() {
  console.warn("Ops Dashboard: Initializing...");
      ...
      if (res.success) {
        ...
        if (heartbeatLabel && heartbeatIcon) {
          if (stressRatio > 0.15) {
            ...
            heartbeatIcon.className =
              "fa-solid fa-triangle-exclamation text-danger fs-5 pulsing-icon";
          } else {
            // ONLINE (Green)
...
function formatActivityText(act) {
  if (act.type === "FILE_CREATED")
    return `New File opened for <strong>${act.description}</strong>`;
  if (act.type === "OT_GENERATED")
    return `Transit Order generated for ${act.description}`;
  if (act.type === "DN_ISSUED")
    return `Delivery Note issued for ${act.description}`;
