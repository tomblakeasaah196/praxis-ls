...
**Next up for another engineer:** PR 7 (the manifest‑coverage gate + remaining manifests), then PR 6 (conversation management/Spaces), and PR 8 (eval harness/observability). PR 5's backend reliability core (E1 timeouts, E2 cheap auto‑continue, E4 embeddings health) landed in [#419](https://github.com/tomblakeasaah196/praxis-ls/pull/419); its FRONTEND part — routing the per‑screen `askPraxis` copilot through the SSE path (E1) and the client fetch timeout (G2) — is a self‑contained frontend follow‑up. The write-contract backlog (154 writes) lives in `src/services/ai/write-contract-baseline.json` and is chipped away in any PR. PR 1, PR 2, PR 4 and PR 5 (backend) are closed. **Whoever closes a milestone updates this section** (the milestone row, the finding rows, and Appendix A where relevant).
...
| PR 5 | Reliability, timeouts & performance (E1–E4, G2) | 🟡 partial | E1 (generous, configurable timeouts) + E2 (cheap, tag‑preserving post‑confirm auto‑continue) + E4 (embeddings‑health boot signal) in [#419](https://github.com/tomblakeasaah196/praxis-ls/pull/419); E3 verified already efficient (documented, no change). Frontend streaming migration (E1) + G2 (client fetch timeout) deferred to a frontend follow‑up |
...
| B3, B6, G2–G4, H1–H2, I1–I2/I4, J1–J5 | — | ⬜ | Not started. (E1–E4 closed in [#419](https://github.com/tomblakeasaah196/praxis-ls/pull/419); G2 + E1's frontend streaming migration are a frontend follow‑up; B3 is a model repoint, not code; F1 is "good, preserve" — not a remediation item; G1 done with D1 in [#404](https://github.com/tomblakeasaah196/praxis-ls/pull/404).) |

**Recommended next PR:** PR 7 (module → AI governance — the manifest‑coverage CI gate + the missing manifests). With grounding (PR 1), completeness/model/fallback + one cached prompt (PR 2), the write contract (PR 3), steering + context window (PR 4), and the reliability core (PR 5 backend) landed, PR 7 is the remaining P0‑flavoured guarantee that keeps "everything is connected to AI" true and fails CI on manifest drift. (The self‑contained frontend follow‑up for PR 5 — E1 streaming‑everywhere + G2 — can be picked up any time by a frontend engineer.)
