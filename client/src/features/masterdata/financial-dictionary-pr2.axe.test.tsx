...
import { apiClientMock, authContextMock, renderScreen, apiError } from "@/test/screen-harness";
...
describe("Financial dictionary — Spend tab", () => {
  it("populated state is axe clean", async () => {
    const { container } = renderScreen(<SpendTab id="di1" />, { routes: { "/financial-dictionary/di1/spend": SPEND } });
    ...
    await waitFor(() => expect(screen.getAllByText(/SBX-2026-0007/).length).toBe(2));
    expect(await axe(container)).toHaveNoViolations();
  });

  it("the chart carries a real accessible label, and the same numbers are in a table", async () => {
    renderScreen(<SpendTab id="di1" />, { routes: { "/financial-dictionary/di1/spend": SPEND } });
  ...
  it("loading state is axe clean", async () => {
    const { container } = renderScreen(<SpendTab id="di1" />, { pending: true });
    expect(await axe(container)).toHaveNoViolations();
  });

  it("error state renders an error, not an all-clear empty state", async () => {
    const { container } = renderScreen(<SpendTab id="di1" />, {
      routes: { "/financial-dictionary/di1/spend": apiError(403, "Not permitted") },
    });
  ...
  it("a period with no movement says so instead of drawing an empty chart", async () => {
    ...
    await waitFor(() => expect(screen.getByText(/No spend in this period/)).toBeTruthy());
    expect(await axe(container)).toHaveNoViolations();
  });
});
...
describe("Financial dictionary — Cost & evolution tab", () => {
  it("populated state is axe clean and the timeline is labelled", async () => {
    const { container } = renderScreen(<CostEvolutionTab id="di1" />, { routes: { "/financial-dictionary/di1/rate-history": RATES } });
    await waitFor(() => expect(screen.getByText(/MAERSK/)).toBeTruthy());
    expect(screen.getByRole("img", { name: /Rate history in XAF/ })).toBeTruthy();
    expect(await axe(container)).toHaveNoViolations();
  });
  ...
  it("an item with no rate cards gets an empty state that says what to do", async () => {
    const { container } = renderScreen(<CostEvolutionTab id="di1" />, {
      routes: { "/financial-dictionary/di1/rate-history": { ...RATES, series: [], trend: { first: null, last: null, delta: null, delta_pct: null, direction: "flat" as const, points: 0 } } },
    });
  ...
  it("error state is axe clean", async () => {
    const { container } = renderScreen(<CostEvolutionTab id="di1" />, {
      routes: { "/financial-dictionary/di1/rate-history": apiError(500, "Upstream failure") },
    });
    const alert = await screen.findByRole("alert");
...
describe("Financial dictionary — import modal", () => {
  it("the opened modal is axe clean and the hidden file input keeps its name", async () => {
    const { container } = renderScreen(<DictImportModal open onClose={() => {}} onImported={() => {}} />);
    await waitFor(() => expect(screen.getByText(/Download template/)).toBeTruthy());
    ...
    expect(screen.getByLabelText(/Upload a filled dictionary template/)).toBeTruthy();
    expect(await axe(container)).toHaveNoViolations();
  });

  it("renders nothing when closed", () => {
    const { container } = renderScreen(<DictImportModal open={false} onClose={() => {}} onImported={() => {}} />);
