...
const BLANK: Row = {
  code: "", label_fr: "", label_en: "", weight: "0", min_duration_hours: "0",
  owner_tier: "INTERNAL", is_anchor: false, is_target_lock: false,
  is_client_visible: true, is_optional: false, chain_segment: "MAIN",
  cadence: "", required_evidence_doc_type: "", auto_advance_on_event: "",
};
...
const segmentOk = (seg: string, total: number) => (seg === "STEADY" ? total === 0 : total === 100);
...
function driftCount(rows: Row[], shipped: api.MilestoneStage[]) {
  ...
  let n = 0;
  if (rows.length !== shipped.length) n += Math.abs(rows.length - shipped.length);
  for (const r of rows) {
...
export function TemplateForm({
  ...
  const shipped = useResource(() => api.milestoneSystemDefault(svc.service_type_id), [svc.service_type_id]);
  ...
  const removeRow = (i: number) => setRows((s) => (s || []).filter((_, ix) => ix !== i));
    ...
    try {
      await api.publishMilestoneTemplate({
        ...
        stages: list.map((s, i) => ({
          ...
          required_evidence_doc_type: s.required_evidence_doc_type.trim() || undefined,
  ...
  return (
    <Modal
      ...
      headerRight={
        drift > 0 ? (
          <Pill tone="warn">{drift === 1 ? "1 change from default" : `${drift} changes from default`}</Pill>
        ) : shipped.data?.length ? (
        ...
        <form className="space-y-4" onSubmit={submit}>
          ...
          <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border px-3 py-2">
            ...
            <span className="ml-auto micro">
              {list.length} {list.length === 1 ? "stage" : "stages"} (allowed {MIN_STAGES}–{MAX_STAGES})
            </span>
          </div>
            ...
            {list.map((r, i) => (
              <div key={i} className="rounded-lg border border-border">
                ...
                <div className="grid items-center gap-2 px-2 py-1.5 sm:grid-cols-[auto_2fr_3fr_auto_auto_auto]">
                  ...
                  <Select
                    value={r.owner_tier}
                    onChange={(e) => setRow(i, { owner_tier: e.target.value as api.OwnerTier })}
                  ...
                  <div className="flex items-center justify-end gap-0.5">
                    ...
                    <button type="button" onClick={() => setOpen(open === i ? null : i)}
                      ...
                      className="px-1 text-muted-foreground hover:text-foreground">{open === i ? "▾" : "▸"}</button>
                    <button type="button" onClick={() => removeRow(i)} disabled={list.length <= MIN_STAGES}
                ...
                {open === i && (
                  <div className="grid gap-3 border-t border-border px-3 py-3 sm:grid-cols-2 lg:grid-cols-3">
                    <Field label="Label (English)">
                      <Input value={r.label_en} onChange={(e) => setRow(i, { label_en: e.target.value })} placeholder="Vessel arrived" />
                    </Field>
                    <Field
                    ...
                    >
                      <Input
                        ...
                        onChange={(e) => setRow(i, { min_duration_hours: e.target.value.replace(/[^0-9]/g, "") })}
                        className="num"
                      />
                    </Field>
                    <Field label="Segment" hint="MAIN for a normal chain; INBOUND / STEADY / OUTBOUND for open-ended services.">
                      <Input value={r.chain_segment} onChange={(e) => setRow(i, { chain_segment: e.target.value.toUpperCase() })} />
                    </Field>
                    <Field label="Cadence" hint="Set only for steady-state stages, which run on a rhythm and are never overdue.">
                      <Select value={r.cadence} onChange={(e) => setRow(i, { cadence: e.target.value })}>
                    ...
                    <Field label="Required evidence" hint="Document type that proves this stage happened.">
                      <Input value={r.required_evidence_doc_type} onChange={(e) => setRow(i, { required_evidence_doc_type: e.target.value.toUpperCase() })} placeholder="POD" />
                    </Field>
                    <Field label="Completed automatically by" hint="An event key that completes this stage when it fires, e.g. delivery_note.created.">
                      <Input value={r.auto_advance_on_event} onChange={(e) => setRow(i, { auto_advance_on_event: e.target.value })} placeholder="delivery_note.created" />
                    </Field>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button type="button" variant="outline" size="sm" onClick={addRow} disabled={list.length >= MAX_STAGES}>
              Add stage
            </Button>
            {shipped.data?.length ? (
              <Button
          ...
          {!canPublish && (
            <ul className="space-y-1 micro">
              {!balanced && <li>Each segment&apos;s weights must total 100 — they are the share of the file&apos;s time each stage gets.</li>}
              {!named && <li>Every stage needs a code and a French label.</li>}
              {!counted && <li>A chain must have between {MIN_STAGES} and {MAX_STAGES} stages.</li>}
              {locks === 0 && <li>Mark one stage as the SLA date, so there is a commitment to protect.</li>}
            </ul>
          )}
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" loading={busy} disabled={!canPublish}>Publish new version</Button>
          </div>
        </form>
      )}
    </Modal>
