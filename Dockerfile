...
#
# `npm ci`, deliberately — unlike the clientbuild stage above, whose lockfile was
# generated on Windows and therefore has to be installed with `npm install`. This
# lockfile records every platform optional for esbuild and rollup (48 entries:
# win32-x64/ia32/arm64, linux-x64-gnu AND -musl, darwin, android…), because it was
# generated by a plain `npm install` on Linux rather than pruned per-platform. So
